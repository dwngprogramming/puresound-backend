...
public class JwtAuthenticationTokenConverter implements Converter<Jwt, AbstractAuthenticationToken> {
    ...
    public AbstractAuthenticationToken convert(Jwt jwt) {
        ...
        if (authorities == null || authorities.isEmpty()) {
            principal = new UserPrincipal(userId, email, fullname, UserType.fromString(userType), List.of());
        } else {
            principal = new UserPrincipal(userId, email, fullname, UserType.fromString(userType), jwtAuthorities);
        }
